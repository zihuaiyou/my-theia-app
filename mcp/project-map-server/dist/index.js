...
// ==================== 服务端初始化 ====================
const server = new Server({ name: "project-map-server", version: "1.0.0" }, { capabilities: { tools: {} } });
 ...
 * 排序规则：目录优先，同级按字母序排列
 */
function scanDir(dirPath, rootPath, currentDepth, maxDepth, excludePatterns) {
    // 超过最大深度，停止递归
 ...
 */
function inferPurpose(filePath, firstLine, exports) {
    ...
    if (name === ".eslintrc.js" || name === ".eslintrc.cjs")
        return "Linting rules";
    if (name === "next.config.js" || name === "next.config.ts")
        return "Next.js configuration";
    if (name === "tailwind.config.ts" || name === "tailwind.config.js")
        return "Tailwind CSS configuration";
    if (name === "vitest.config.ts" || name === "jest.config.ts")
        return "Test configuration";
    if (name === "docker-compose.yml" || name === "docker-compose.yaml")
        return "Docker service orchestration";
    if (name === "Dockerfile")
        return "Container image definition";
    // —— 源码文件，根据目录名推断 ——
    if (dir.includes("pages") || dir.includes("app/router"))
        return "Page component / route handler";
    if (dir.includes("components") || dir.includes("Component"))
        return "UI component";
    if (dir.includes("lib") || dir.includes("utils") || dir.includes("helpers"))
        return "Utility / helper functions";
    if (dir.includes("hooks"))
        return "React hooks";
    if (dir.includes("stores") || dir.includes("store"))
        return "State management";
    if (dir.includes("types") || dir.includes("interfaces"))
        return "Type definitions";
    if (dir.includes("api") && !dir.includes("component"))
        return "API client / server handler";
    if (dir.includes("middleware"))
        return "Middleware";
    if (dir.includes("styles") || dir.includes("css"))
        return "Styles / theme";
    // —— 兜底：用 exports 或文件首行内容 ——
    if (exports.length > 0)
        return `Exports: ${exports.slice(0, 3).join(", ")}${exports.length > 3 ? "..." : ""}`;
    if (firstLine)
 ...
 * - 限制最多处理 200 个文件
 */
async function handleAnalyzeKeyFiles(args) {
    const rootPath = args.rootPath;
    if (!fs.existsSync(rootPath)) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "path_not_found", message: `Path not found: ${rootPath}` }) }] };
    }
...
function detectProjectType(pkg) {
    ...
    if (pkg.private && pkg.scripts?.build)
        return "app";
    if ((pkg.main ?? pkg.module ?? pkg.exports) && pkg.name)
        return "library";
    return "other";
}
/** 将依赖名归类 */
...
/** 从依赖列表中检测框架 */
function detectFramework(pkgDeps) {
...
function extractImports(content, filePath, rootPath) {
    const imports = [];
    let match;
...
/** 根據目錄名推斷用途 */
function inferDirPurpose(dir) {
    const d = dir.toLowerCase().replace(/\\/g, "/");
    if (/components?/.test(d))
        return "UI 元件";
    if (/pages?/.test(d))
        return "頁面路由";
    if (/app\/router/.test(d) || /\bapp$/.test(d))
        return "App Router 頁面";
    if (/lib/.test(d) || /utils?/.test(d))
        return "工具函數";
    if (/hooks?/.test(d))
        return "React Hooks";
    if (/stores?/.test(d) || /state/.test(d))
        return "狀態管理";
    if (/types?/.test(d) || /interfaces/.test(d))
        return "型別定義";
    if (/api/.test(d))
        return "API 處理";
    if (/middleware/.test(d))
        return "中介軟體";
    if (/styles?/.test(d) || /css/.test(d) || /theme/.test(d))
        return "樣式";
    if (/config/.test(d))
        return "配置";
    if (/layouts?/.test(d))
        return "佈局";
    if (/\btests?\b/.test(d) || /__tests__/.test(d) || /spec/.test(d))
        return "測試";
    return undefined;
}
/** 根據目錄名生成架構規則 */
function generateRule(dir, pattern) {
    ...
    if (template)
        return template(pattern.dir, naming);
    // 兜底：通用規則
 ...
 * - 生成架構規則
 */
async function handleExtractArchPatterns(args) {
    const rootPath = args.rootPath;
    if (!fs.existsSync(rootPath)) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "path_not_found", message: `Path not found: ${rootPath}` }) }] };
    }
    ...
    for (const s of summaries) {
        for (const imp of s.imports) {
            ...
            if (targetDir && targetDir !== s.dir) {
                if (!importGraph.has(s.dir))
                    importGraph.set(s.dir, new Set());
    ...
    for (const [dir, files] of dirMap) {
        ...
        // 取主導命名慣例
        const dominantNaming = Object.entries(namingCounts).sort((a, b) => b[1] - a[1])[0][0];
    ...
    const result = { dirPatterns, rules };
    return {
        content: [{ type: "text", text: JSON.stringify(result) }],
    };
}
